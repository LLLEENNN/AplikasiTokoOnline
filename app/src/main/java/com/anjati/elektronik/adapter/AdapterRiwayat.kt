...
class AdapterRiwayat(var data: ArrayList<Transaksi>, var listener: Listeners) : RecyclerView.Adapter<AdapterRiwayat.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        val name = a.details[0].product.name
        holder.tvNama.text = a.kode_trx
        holder.tvHarga.text = Helper().gantiRupiah(Integer.valueOf(a.total_transfer) + Integer.valueOf(a.kode_unik))
...
//        holder.tvTangal.text = Helper().convertTanggal(a.created_at, formatBaru)
        holder.tvTangal.text = Helper().dateFormat(a.created_at) + " Pukul " + Helper().timeFormat(a.created_at)
